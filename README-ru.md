...
- иерархичность узлов удобна для применения стилей;
...
Кроме того, BookFx поддерживает прототипирование. Это значит, что вы можете использовать части xlsx-файлов в качестве компонентов. Слоты тоже поддерживаются.
...
Класс `Make` это точка входа BookFx. Он предоставляет методы для создания книг, листов, box'ов, стилей и границ. Точка выхода это `ToBytes()`.
...
Полная версия в примерах использования, см. ниже.
