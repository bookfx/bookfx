...
- `protoBook` – это `byte[]` с xlsx-файлом;
- `"Prototype1"` – имя диапазона в этом файле, который будет использоваться в качестве прототипа;
- `"Slot1"` и `"Slot2"` – имена диапазонов внутри прототипа, в которых могут быть размещены box'ы.

Одна книга может содержать несколько прототипов. Один прототип может использоваться в разных `ProtoBox`'ах. Каждый `ProtoBox` может использоваться многократно.

Кроме описанного выше механизма прототипирования с помощью `ProtoBox` BookFx поддерживает добавление целых листов из других книг.

```c#
Make.Sheet("New Sheet Name", protoBook, "Prototype Sheet Name");
```

В результате из xlsx-файла `protoBook` будет скопирован лист `"Prototype Sheet Name"`, которому будет присвоено имя `"New Sheet Name"`. См. также другие перегрузки `Make.Sheet`.
